...
{#if !open}
	<div
		class="fixed bottom-11 z-[60] w-full md:hidden"
		in:fly={{ y: 40, duration: 220, opacity: 0 }}
		out:fly={{ y: 40, duration: 220, opacity: 0 }}
	>
		<div
			class="w-full border-t border-border/60 bg-background/95 backdrop-blur-md supports-backdrop-filter:bg-background/80"
		>
			<SeekSlider
				class="h-3"
				growOnHover={false}
	...
	>
		...
		<div class="relative flex min-h-0 flex-1 flex-col">
			...
			>
				...
				<p
					class="truncate text-sm font-medium tracking-widest text-white/70 uppercase"
				>
					Now Playing
				</p>
				{#if currentMediaItem}
			...
			>
				<div class="flex-1 overflow-y-auto overscroll-contain px-6">
					<div class="flex min-h-full flex-col justify-center gap-4 py-4">
						...
						<!-- Seek bar -->
						<div class="flex w-full flex-col gap-0.5" data-no-drag>
							<SeekSlider
								value={seekValue}
								onValue={(p) => {
									musicManager.setPosition(p * musicManager.duration);
							...
							/>
							<div
								class="flex justify-between text-xs text-white/60 tabular-nums"
							>
						...
						>
							...
							{#if musicManager.loading}
								<div
									class="h-16 w-16 animate-pulse rounded-full bg-white/15"
								></div>
							{:else if musicManager.playing}
								<button
					...
					<div class="flex flex-col gap-3">
						...
						{#if currentQueueItem}
							<div>
								...
								<div
									class="flex items-center gap-3 rounded-md bg-accent/50 p-3"
								>
