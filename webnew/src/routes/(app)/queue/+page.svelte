    ...
    <div class="flex flex-col gap-1">
      {#each items as item, index (item.queueItemId)}
        ...
        {#if index === 0 && currentIndex > 0}
          <p
            class="px-2 pb-1 pt-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground"
          >
            Played
          </p>
        {/if}
        ...
        >
          <span
            class="hidden w-6 flex-shrink-0 text-right text-xs tabular-nums text-muted-foreground sm:block sm:w-8 sm:text-sm"
          >
            {index + 1}
          </span>
          ...
          <div class="flex min-w-0 flex-1 flex-col">
            <p class="truncate text-sm font-medium sm:text-base">{item.name}</p>
          ...
          <span
            class="hidden flex-shrink-0 text-xs tabular-nums text-muted-foreground sm:block sm:text-sm"
          >
          ...
          {:else if isCurrent}
            <Play class="ml-1 h-4 w-4 flex-shrink-0 text-primary sm:ml-2 sm:h-5 sm:w-5" />
          {:else}
            <Play
