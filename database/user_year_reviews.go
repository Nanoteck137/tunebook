...
type GenerateUserReviewTotalParams struct {
	UserId string
}

func (db *Database) GenerateUserReviewTotal(
	ctx context.Context,
	params GenerateUserReviewTotalParams,
) error {
	now := time.Now().UnixMilli()
	_ = now
	...
	// TODO(patrik): Get the data and insert
	// _, err = tx.Exec(ctx, dialect.Insert(userYearReviewsTbl).Rows(
	// 	goqu.Record{
	// 		"user_id":        params.UserId,
	// 		"year":           0,
	...
	// TODO(patrik): Replace with function to get generate the total
	// err = tx.generateUserYearReviewTracks(
	// 	ctx, params.UserId, params.Year, now)
	// if err != nil {
	// 	return err
