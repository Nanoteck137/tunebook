...
func TestPlannerErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: "operator 'contains' cannot be used on integer field",
		},
		{
			name:    "like on int",
			input:   `year like "%1970%"`,
			wantErr: "operator 'like' cannot be used on integer field",
		},
		{
			name:    "greater on string",
			input:   `name > "rock"`,
			wantErr: "operator '>' cannot be used on string field",
		},
		{
		...
		},
		{
			...
			input:   `name is null`,
			wantErr: "field 'name' is not nullable",
		},
		{
		...
		},
		{
			...
			wantErr: "operator 'contains' cannot be used on float field",
		},
		{
			name:    "greater on bool",
			input:   `active > true`,
			wantErr: "operator '>' cannot be used on boolean field",
		},
	}
