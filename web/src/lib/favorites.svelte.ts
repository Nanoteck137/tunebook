...
class Favorites {
  ...
  async toggleTrack(trackId: string) {
    if (this.hasTrack(trackId)) {
      const res = await this.apiClient.removeUserFavorite(trackId);
      ...
      const res = await this.apiClient.addUserFavorite(trackId);
    ...
    const ids = await this.apiClient.getUserFavoritesIds();
    if (!ids.success) {
